...
static int32_t msm_actuator_get_eeprom_data(struct msm_actuator_ctrl_t *o_ctrl, uint8_t *data)
{
	...
	/*
	 * Read the OIS calibration data straight into the caller's buffer.
	 * Reading it into a local and then assigning that local to the 'data'
	 * parameter only reassigned the pointer, so the caller was left
	 * programming its own uninitialised buffer into the OIS.
	 */
	rc = o_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
			&o_ctrl->i2c_client, oisCali_reg, data,
			OIS_CALI_DATA_SIZE);
	if (rc < 0)
		pr_err("Read OIS calibration data failed, rc:%d\n", rc);
